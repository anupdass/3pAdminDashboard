...
import { useGetAllSeListQuery } from '../../redux/features/seSlice';
import SeDetailsModal from './SeDetailsModal';
...
const SEList = () => {
    ...
    const { data, isLoading, isError, error } = useGetAllSeListQuery();
    ...
    const totals = data?.reduce((acc, entry) => ({
        constingAmount: acc.constingAmount + entry.constingAmount,
        paymentBCBL: acc.paymentBCBL + entry.paymentBCBL,
        paymentHand: acc.paymentHand + entry.paymentHand
    }), { constingAmount: 0, paymentBCBL: 0, paymentHand: 0 });


    const totalPaymentDue = totals?.constingAmount - totals?.paymentBCBL - totals?.paymentHand;
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {data?.length > 0 ? (
                                    data.map((entry, index) => {
                                        const paymentDue = calculatePaymentDue(entry.constingAmount, entry.paymentBCBL, entry.paymentHand);
                                        return (
                                            <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-right text-red-700 font-semibold">
                                                    ৳ {entry.constingAmount.toLocaleString()}
                                                </td>
                                                <td className="px-4 py-3 text-right text-slate-700">
                                                    ৳ {entry.paymentBCBL.toLocaleString()}
                                                </td>
                                                <td className="px-4 py-3 text-right text-slate-700">
                                                    ৳ {entry.paymentHand.toLocaleString()}
                                                </td>
                                ...
                                {data?.length > 0 && (
                                    <tr className="bg-slate-100 font-bold border-t-2 border-slate-300">
                                        ...
                                        <td className="px-4 py-3 text-right text-red-700">
                                            ৳ {totals?.constingAmount.toLocaleString()}
                                        </td>
                                        <td className="px-4 py-3 text-right text-slate-700">
                                            ৳ {totals.paymentBCBL.toLocaleString()}
                                        </td>
                                        <td className="px-4 py-3 text-right text-slate-700">
                                            ৳ {totals?.paymentHand.toLocaleString()}
                                        </td>
                                        <td className="px-4 py-3 text-right">
